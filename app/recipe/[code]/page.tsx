...
export async function generateMetadata({ params, searchParams }: FoodSharePageProps): Promise<Metadata> {
  ...
  const preview = await resolveFoodSharePreviewForRoute("recipe", code, await searchParams);
  return buildFoodShareMetadata(createFoodTarget("recipe", code), preview);
}

export default async function RecipePage({ params, searchParams }: FoodSharePageProps) {
  const { code } = await params;
  const target = createFoodTarget("recipe", code);
  const preview = await resolveFoodSharePreviewForRoute("recipe", code, await searchParams);
